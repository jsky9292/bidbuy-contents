...
export default function Dashboard({ draftPosts: initialDrafts, publishedPosts: initialPublished, apiStatus }) {
  ...
  const handleAction = async (postId, action, scheduledAt = null, category = null) => {
    try {
      ...
      alert('?§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§: ' + error.message);
    }
  };

  const handleDelete = async (slug) => {
    if (!confirm('?ïÎßêÎ°????¨Ïä§?∏Î? ??†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) return;
    try {
  ...
  const handleSchedule = (postId) => {
    const scheduledAt = prompt('?àÏïΩ ?úÍ∞Ñ???ÖÎ†•?òÏÑ∏??(YYYY-MM-DD HH:MM):');
  ...
  return (
    <Layout title="Í¥ÄÎ¶¨Ïûê">
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 py-8">
          {/* ?§Îçî */}
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">?Ä?úÎ≥¥??/h1>
              <p className="text-sm text-gray-500 mt-1">ÏΩòÌÖêÏ∏†Î? Í¥ÄÎ¶¨Ìïò?∏Ïöî</p>
            </div>
          ...
          {(!apiStatus.youtube || !apiStatus.gemini) && (
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-amber-800">API ???§Ï†ï ?ÑÏöî</p>
                  <p className="text-sm text-amber-600 mt-1">
                    ?úÏä§?úÏùÑ ?¨Ïö©?òÎ†§Î©?API ???§Ï†ï???ÑÏöî?©Îãà??{' '}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
            <Link href="/admin/consultations">
              <div className="bg-teal-50 rounded-xl p-5 border border-teal-100 hover:border-teal-200 hover:shadow-sm transition-all cursor-pointer">
                <div className="text-sm text-teal-600 mb-1">Î¨∏Ïùò Í¥ÄÎ¶?/div>
                <div className="text-2xl font-bold text-teal-700">?ìã Î≥¥Í∏∞</div>
              </div>
            </Link>
            <Link href="/admin/posts">
              <div className="bg-green-50 rounded-xl p-5 border border-green-100 hover:border-green-200 hover:shadow-sm transition-all cursor-pointer">
                <div className="text-sm text-green-600 mb-1">?¨Ïä§??Í¥ÄÎ¶?/div>
                <div className="text-2xl font-bold text-green-700">?ìù {publishedPosts.length}Í∞?/div>
              </div>
            </Link>
            <Link href="/">
              <div className="bg-white rounded-xl p-5 border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all cursor-pointer">
                <div className="text-sm text-gray-500 mb-1">Î∏îÎ°úÍ∑?/div>
                <div className="text-2xl font-bold text-gray-900">Î≥¥Í∏∞ ??/div>
              </div>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Í≤Ä???ÄÍ∏?/h2>
                <span className="text-sm text-gray-400">{draftPosts.length}Í∞?/span>
              </div>

              {draftPosts.length === 0 ? (
                <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
                  <p className="text-gray-400 mb-4">Í≤Ä?†Ìï† ?¨Ïä§?∏Í? ?ÜÏäµ?àÎã§</p>
                  <Link href="/admin/discover">
                    <button className="px-4 py-2 bg-gray-900 text-white text-sm font-medium rounded-lg hover:bg-gray-800 transition-colors">
                      ?ÅÏÉÅ Í≤Ä?âÌïòÍ∏?                    </button>
                  </Link>
                </div>
              ) : (
                  ...
                  {draftPosts.map((post) => (
                    <div key={post.id} className="bg-white rounded-xl border border-gray-100 p-5 hover:border-gray-200 transition-colors">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button
                          onClick={() => { if (confirm('??†ú?òÏãúÍ≤†Ïäµ?àÍπå?')) handleAction(post.id, 'reject'); }}
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-semibold text-gray-900">ÏµúÍ∑º Î∞úÌñâ</h2>
                <span className="text-sm text-gray-400">{publishedPosts.length}Í∞?/span>
              </div>

              {publishedPosts.length === 0 ? (
                <div className="bg-white rounded-xl border border-gray-100 p-8 text-center">
                  <p className="text-gray-400">Î∞úÌñâ???¨Ïä§?∏Í? ?ÜÏäµ?àÎã§</p>
                </div>
              ) : (
                <div className="space-y-3">
...
export async function getServerSideProps() {
  try {
    ...
    console.error('Í¥ÄÎ¶¨Ïûê ?Ä?úÎ≥¥???∞Ïù¥??Ï°∞Ìöå ?§Ìå®:', error);
    return {
      props: {
        draftPosts: [],
        publishedPosts: [],
